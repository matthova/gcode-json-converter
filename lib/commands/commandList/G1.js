const G1 = async (bot, gcodeObject) => {
  ...
  gcodeObject.args.forEach((arg) => {
    if (arg.indexOf('X') !== -1) {
      bot.position.x.latest = Number(arg.split('X')[1], 10);
    }
    if (arg.indexOf('Y') !== -1) {
      bot.position.y.latest = Number(arg.split('Y')[1], 10);
    }
    if (arg.indexOf('Z') !== -1) {
      bot.position.z.latest = Number(arg.split('Z')[1], 10);
    }
    if (arg.indexOf('E') !== -1) {
      bot.position.e.latest = Number(arg.split('E')[1], 10);
    }
  });
